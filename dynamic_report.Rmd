...
```
```{r rename variables}
# Remove second column (which asked for manual time stamp input)
logged_data <- logged_data[-2]

# Renaming the variables in the `logged_data` dataframe.
...
logged_data <- logged_data %>%
  mutate(bird_species = case_when(str_detect(bird_species, "Sparrow") ~ "Sparrow",
                                  str_detect(bird_species, "Blackbird") ~ "Blackbird",
                                  str_detect(bird_species, "Chaffinch") ~ "Chaffinch",
                                  str_detect(bird_species, "Rosella") ~ "Rosella",
                                  str_detect(bird_species, "Pidgeon") ~ "Pigeon",
                                  TRUE ~ bird_species))
...
mean_temp <- mean(logged_data$temperature)
...
weather_most <- regmatches(logged_data$weather, gregexpr(",", logged_data$weather)) %>%
  lengths() %>%
  max()

weather_most_words <- logged_data$weather[which(lengths(regmatches(logged_data$weather, gregexpr(", ", logged_data$weather))) == weather_most)][1]

# The number of unique bird species spotted was:
unique_birds <- logged_data$bird_species %>%
  unique() %>%
  length()
```
...
bird_count <- table(logged_data$bird_species)
